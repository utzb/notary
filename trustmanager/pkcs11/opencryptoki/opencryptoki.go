...
func (ks *keyStore) HardwareListKeys(ctx universal.IPKCS11Ctx, session pkcs11.SessionHandle) (keys map[string]universal.HardwareSlot, err error) {
	...
	for _, obj := range objs {
		...
		keys[data.NewECDSAPublicKey(pubBytes).ID()] = universal.HardwareSlot{
...
func (ks *keyStore) listObjects(ctx universal.IPKCS11Ctx, session pkcs11.SessionHandle) ([]pkcs11.ObjectHandle, error) {
	findTemplate := []*pkcs11.Attribute{
		pkcs11.NewAttribute(pkcs11.CKA_TOKEN, true),
		pkcs11.NewAttribute(pkcs11.CKA_CLASS, pkcs11.CKO_CERTIFICATE),
		pkcs11.NewAttribute(pkcs11.CKA_CERTIFICATE_TYPE, pkcs11.CKC_X_509),
