...
type HardwareSpecificStore interface {
	Name() string
	AddECDSAKey(IPKCS11Ctx, pkcs11.SessionHandle, data.PrivateKey, []byte, notary.PassRetriever, data.RoleName) error
	GetECDSAKey(IPKCS11Ctx, pkcs11.SessionHandle, []byte) (*data.ECDSAPublicKey, data.RoleName, error)
	Sign(IPKCS11Ctx, pkcs11.SessionHandle, []byte, notary.PassRetriever, []byte) ([]byte, error)
	HardwareRemoveKey(IPKCS11Ctx, pkcs11.SessionHandle, []byte, notary.PassRetriever, string) error
